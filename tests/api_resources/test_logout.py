...
class TestLogout:
    ...
    def test_method_perform(self, client: Papr) -> None:
        logout = client.logout.perform()
        assert_matches_type(LogoutPerformResponse, logout, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    def test_raw_response_perform(self, client: Papr) -> None:
        response = client.logout.with_raw_response.perform()
    ...
    def test_streaming_response_perform(self, client: Papr) -> None:
        with client.logout.with_streaming_response.perform() as response:
...
class TestAsyncLogout:
    ...
    async def test_method_perform(self, async_client: AsyncPapr) -> None:
        logout = await async_client.logout.perform()
        assert_matches_type(LogoutPerformResponse, logout, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_raw_response_perform(self, async_client: AsyncPapr) -> None:
        response = await async_client.logout.with_raw_response.perform()
    ...
    async def test_streaming_response_perform(self, async_client: AsyncPapr) -> None:
        async with async_client.logout.with_streaming_response.perform() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
